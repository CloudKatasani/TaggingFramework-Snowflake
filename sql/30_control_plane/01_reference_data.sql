...
CREATE OR REPLACE VIEW REF_OPERATING_COMPANY COMMENT = 'Active operating companies.' AS
SELECT VALUE_CODE AS OPERATING_COMPANY, DISPLAY_NAME,
       ATTRIBUTES:legal_entity_id::STRING AS LEGAL_ENTITY_ID,
       ATTRIBUTES, VALID_FROM, VALID_TO
FROM REFERENCE_VALUE
WHERE REFERENCE_SET = 'REF_OPERATING_COMPANY' AND IS_ACTIVE
  AND CURRENT_DATE() BETWEEN VALID_FROM AND COALESCE(VALID_TO, '9999-12-31'::DATE);

-- Teams change far too often for ALLOWED_VALUES, and a disbanded team must stop
-- being a legal owner the day it is dissolved - which is exactly what an
-- effective-dated reference set gives you and a static vocabulary does not.
CREATE OR REPLACE VIEW REF_TEAM COMMENT = 'Active engineering teams.' AS
SELECT VALUE_CODE AS TEAM, PARENT_CODE AS DEPARTMENT, DISPLAY_NAME,
       ATTRIBUTES:manager::STRING AS TEAM_MANAGER,
       ATTRIBUTES:support_group::STRING AS SUPPORT_GROUP, ATTRIBUTES
FROM REFERENCE_VALUE
WHERE REFERENCE_SET = 'REF_TEAM' AND IS_ACTIVE
  AND CURRENT_DATE() BETWEEN VALID_FROM AND COALESCE(VALID_TO, '9999-12-31'::DATE);
...
SELECT VALUE_CODE AS COST_CENTER, PARENT_CODE AS OPERATING_COMPANY, DISPLAY_NAME,
...
CREATE TABLE IF NOT EXISTS DOMAIN_OWNERSHIP (
    ...
    OPERATING_COMPANY STRING,           -- NULL = the domain spans all opcos
